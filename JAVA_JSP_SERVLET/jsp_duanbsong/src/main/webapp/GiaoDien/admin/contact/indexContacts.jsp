                               		...
                               		if(listContacts != null || listContacts.size() > 0){
                               			for(Contacts objContact : listContacts){ %>
                                			<tr>
                                        		...
                                        		<%if("admin".equals(userLogin.getUsername())){ %>                                     
                                        		<td class="center">
                                            	<a href="<%=request.getContextPath()%>/admin/contacts/edit?id=<%=objContact.getIdContact()%>" title="" class="btn btn-primary"><i class="fa fa-edit "></i> Sửa</a>
...
<script>
	function showMess(id) {
		...
		if(option){
			location.href = "<%=request.getContextPath()%>/admin/contacts/del?id="+id;
		}
	}
</script>
<script>
